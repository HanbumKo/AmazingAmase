...
if __name__ == '__main__':
    ...
    for i in range(number_recoveryzone):
        ...
        plt.text(point_list[i + number_keepinzone][0], point_list[i + number_keepinzone][1], "{}".format(i), fontsize=8)
    ...
    for i in range(len(findpointlist)):
        plt.triplot(findpointlist[i][:, 0], findpointlist[i][:, 1], trilist[i].simplices.copy())

    '''
    VISUALIZATION
    '''
    for i in range(len(findpointlist)):
        plt.plot(findpointlist[i][:,0], findpointlist[i][:,1], 'o')
        for j in range(len(findpointlist[i])):
            plt.text(findpointlist[i][j][0], findpointlist[i][j][1]-0.01*i, "{},{}".format(i,j),fontsize = 8)

    plt.xlim([point_list[2][0] - 0.05, point_list[1][0] + 0.05])
    plt.ylim([point_list[0][1] - 0.05, point_list[1][1] + 0.05])
    # plt.xlim([point_list[2][0], point_list[1][0]])
    # plt.ylim([point_list[0][1], point_list[1][1]])

    plt.title('Voronoi -> Delaunay')
    plt.savefig('voro'+str(number_recoveryzone)+'.png')
    plt.show()